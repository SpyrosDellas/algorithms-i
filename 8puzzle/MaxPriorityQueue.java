 ...
 *  Name: Spyros Dellas
 *  Date: 12/05/2020
 *  Description: Max oriented Priority Queue implementation using a binary heap
 **************************************************************************** */
...
public class MaxPriorityQueue<K extends Comparable<K>> {
    ...
    // Certify that this is a max oriented priority queue
    private boolean isPQ() {
        for (int k = 1; k <= size / 2; k++) {
            int j = 2 * k;
            if (less(k, j) || (j < size && less(k, 2 * k + 1)))
                return false;
        }
        return true;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            t[i] = k.multiply(k).multiply(k);
            k = k.add(BigInteger.ONE);
        }

        System.out.println("Big Integer array created");
        MaxPriorityQueue<BigInteger> pq = new MaxPriorityQueue<>(t);
        for (int i = 1; i <= 7; i++) {
            System.out.print(pq.delMax() + " ");
        }
        System.out.println("\nCertified as a max oriented priority queue: " + pq.isPQ());
    }
}
